...
require_relative '../paylocity_api'

class PaylocityApi::Encryption
  ALG = "AES-256-CBC".freeze

  def self.encode(data)
    if PaylocityApi.config.encryption_key_path.nil?
    ...
    public_key = OpenSSL::PKey::RSA.new(File.read(PaylocityApi.config.encryption_key_path))
