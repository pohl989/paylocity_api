...
class PaylocityApi::Collection
  ...
  def initialize(data: [], model_class: nil, path: nil)
    @data           = data
    @model_class    = model_class || @data.first&.class || raise(ArgumentError.new('PaylocityApi::Collection initialization requries model_class if data is empty'))
    @path           = path
  end
  ...
  def create(data)
    ...
    url = self.path
    if model_class == PaylocityApi::Deduction
      data = { deduction: data }
      data[:deduction][:employeeId] = path.gsub('employees/', '').to_i
      data[:deduction][:companyNumber] = PaylocityApi.config.company_id
      url = 'deduction'
    end
  ...
  def client
    @client ||= PaylocityApi::Client.new
  end
end